...
bool    ProtocolParse_Writer_TypeScript::CreateFile_TScript_NSV(string foldername, int filetype, bool bRefProtocol)
{
	...
	for (size_t i = 0; i < m_NamespaceVec.size(); i++) {
		WriteOneNameSpace_TScript_NSV(m_NamespaceVec[i], filetype, foldername);
	}


	return true;
}
bool    ProtocolParse_Writer_TypeScript::WriteOneNameSpace_TScript_NSV(stNamespace& pNamespace, int filetype, string foldername)
{
	m_tablenum = 0;
	...
	////写函数的id,和协议号
	CreateAndWriteFuncIdAndProtocolNo_TScript_NSV(pNamespace, foldername, tt_strWriteNameSpace);


	//先写枚举
	for (size_t i = 0; i < pNamespace._EnumVec.size(); i++) {
		WriteEnum_TScript_NSV(pNamespace, pNamespace._EnumVec[i], foldername, tt_strWriteNameSpace);
	}
	...
	for (int i = 0; i < totalsize; i++) {
		if ((!bvectorend) && (!bstructend) && (!bwjsvectorend)) {
			if (pNamespace._StructVec[structindex]._index < pNamespace._VectorVec[vectorindex]._index
				...
				//写结构体
				WriteStruct_TScript_NSV(pNamespace, pNamespace._StructVec[structindex], foldername, tt_strWriteNameSpace);
				structindex++;
			}
				...
				//写vector
				WriteVector_TScript_NSV(pNamespace, pNamespace._VectorVec[vectorindex], foldername, tt_strWriteNameSpace);
				vectorindex++;
			}
			else {
				//写类vector
				WriteWJSVector_TScript_NSV(pNamespace, pNamespace._WJSVectorVec[wjsvectorindex], foldername, tt_strWriteNameSpace);
				wjsvectorindex++;
			}
		}
	...
	for (size_t i = 0; i < pNamespace._ClassVec.size(); i++) {
		WriteClass_TScript_NSV(pNamespace._ClassVec[i], filetype, pNamespace, foldername, tt_strWriteNameSpace);
	}

	return true;
}

void    ProtocolParse_Writer_TypeScript::WriteEnum_TScript_NSV(stNamespace& pNamespace, stEnum& pEnum, string foldername, const string& strWriteNameSpace)
{
	char t_ch[512];
	...
	string strTable_1_ = CreateTableString(1);
	string strTable_2_ = CreateTableString(2);

	string strTable_0 = CreateTableString(3);
	string strTable_1 = CreateTableString(4);
	string strTable_2 = CreateTableString(5);
	string strTable_3 = CreateTableString(6);
	string strTable_4 = CreateTableString(7);
	string strTable_5 = CreateTableString(8);
	...
	sprintf(t_ch, "namespace yy{\n%sexport namespace wjsproto{\n%sexport namespace %s{\n", strTable_1_.c_str(), strTable_2_.c_str(),
	...
	strEnum.append(strTable_2_.c_str());
	strEnum.append("}\n");
	strEnum.append(strTable_1_.c_str());
	strEnum.append("}\n");
	strEnum.append("}\n");
...
void    ProtocolParse_Writer_TypeScript::CreateAndWriteFuncIdAndProtocolNo_TScript_NSV(stNamespace& pNamespace, string foldername, const string& strWriteNameSpace)
{
	...
	strid.append(strTable_2_);
	strid.append("}\n");

	strid.append(strTable_1_);
	strid.append("}\n");
...
void    ProtocolParse_Writer_TypeScript::WriteStruct_TScript_NSV(stNamespace& pNamespace, stStructStruct& pStruct, string foldername, const string& strWriteNameSpace)
{
	...
	string strTable_1 = CreateTableString(3);
	string strTable_2 = CreateTableString(4);
	string strTable_3 = CreateTableString(5);
	string strTable_4 = CreateTableString(6);
	string strTable_5 = CreateTableString(7);
	string strTable_6 = CreateTableString(8);
	...
	for (size_t i = 0; i < tt_PacketNameList_Ref.size(); i++) {
	...
	strStruct.append(strTable_2_);
	strStruct.append("}\n");

	strStruct.append(strTable_1_);
	strStruct.append("}\n");
...
void    ProtocolParse_Writer_TypeScript::WriteVector_TScript_NSV(stNamespace& pNamespace, stVector& pVector, string foldername, const string& strWriteNameSpace)
{
	...
	if (pVector._ParTName == "byte" || pVector._ParTName == "BYTE" || pVector._ParTName == "char" || pVector._ParTName == "unsigned char") {
		WriteVector2_TScript_NSV(pNamespace, pVector, foldername, strWriteNameSpace);
		return;
	}
	char t_ch[512];
	...
	sprintf(t_ch, "///<reference path=\"../../Base/Array.ts\"/>\nnamespace yy{\n%sexport namespace wjsproto{\n%sexport namespace %s{\n", strTable_1_.c_str(), strTable_2_.c_str(),
		strWriteNameSpace.c_str());
	...
	strVector.append(strTable_2_);
	strVector.append("}\n");

	strVector.append(strTable_1_);
	strVector.append("}\n");
...
void    ProtocolParse_Writer_TypeScript::WriteVector2_TScript_NSV(stNamespace& pNamespace, stVector& pVector, string foldername, const string& strWriteNameSpace)
{
	if (!pVector._bBiaoZhuUsed)
		return;
	char t_ch[512];
...
void    ProtocolParse_Writer_TypeScript::WriteWJSVector_TScript_NSV(stNamespace& pNamespace, stWJSVector& pWJSVector, string foldername, const string& strWriteNameSpace)
{
	...
	if (pWJSVector._ParTName == "byte" || pWJSVector._ParTName == "BYTE" || pWJSVector._ParTName == "char" || pWJSVector._ParTName == "unsigned char") {
		WriteWJSVector2_TScript_NSV(pNamespace, pWJSVector, foldername, strWriteNameSpace);
		return;
	}
	...
	strWJSVector.append(strTable_2_);
	strWJSVector.append("}\n");

	strWJSVector.append(strTable_1_);
	strWJSVector.append("}\n");
	strWJSVector.append("}\n");
...
void    ProtocolParse_Writer_TypeScript::WriteWJSVector2_TScript_NSV(stNamespace& pNamespace, stWJSVector& pWJSVector, string foldername, const string& strWriteNameSpace)
{
	if (!pWJSVector._bBiaoZhuUsed)
		return;
	char t_ch[512];
...
void    ProtocolParse_Writer_TypeScript::WriteClass_Send_TScript_NSV(stClass& pClass, int filetype, stNamespace& pNamespace, string foldername, const string& strWriteNameSpace)
{
	char t_ch[512];
	...
	for (size_t i = 0; i < pClass._FuncVec.size(); i++) {
		...
		if (filetype == 2 && ((m_bSCSpliteS && pClass._FuncVec[i]._SCSpliteType == 1) || (m_bSCSpliteC && pClass._FuncVec[i]._SCSpliteType == -1)))
			continue;
		//写函数
		WriteFunc_SendByteSeq_TScript_NSV(pClass._FuncVec[i], 3, pClass, pNamespace, foldername, fp);
	}
	...
	strclass.append(strTable_2_);
	strclass.append("}\n");
	strclass.append(strTable_1_);
	strclass.append("}\n");
	strclass.append("}\n");
...
void    ProtocolParse_Writer_TypeScript::WriteFunc_SendByteSeq_TScript_NSV(stFunc& pFunc, int tablenum, stClass& pClass, stNamespace& pNamespace, string foldername, FILE* fp)
{
	char t_ch[1024];
...
void    ProtocolParse_Writer_TypeScript::GetUsePacketNamesByParType_TScript_NSV(const string& strParType, vector<string>& packetNamesList, const string& selffoldername, bool bFirst)
{
	if (m_bTScriptUseNameSpaceVersion || (!bFirst)) {
				...
				if (m_NamespaceVec[i]._StructVec[j]._name.compare(strParType.c_str()) == 0) {
					...
					if (selffoldername.compare(m_NamespaceVec[i]._protocolname.c_str()) == 0)
						...
						if (m_bTScriptUseNameSpaceVersion)
							sprintf(tt_ch, "import %s=%s.%s;", m_NamespaceVec[i]._StructVec[j]._name.c_str(), m_NamespaceVec[i]._protocolname.c_str(), m_NamespaceVec[i]._StructVec[j]._name.c_str());
						else
							sprintf(tt_ch, "import %s=%s.%s;", m_NamespaceVec[i]._StructVec[j]._name.c_str(), m_NamespaceVec[i]._shortname.c_str(), m_NamespaceVec[i]._StructVec[j]._name.c_str());
					...
					for (int k = 0; k < (int)m_NamespaceVec[i]._StructVec[j]._ParVec.size(); k++) {
						if (!IsTScriptBaseType(m_NamespaceVec[i]._StructVec[j]._ParVec[k]._strParType, false, true)) {
							GetUsePacketNamesByParType_TScript_NSV(m_NamespaceVec[i]._StructVec[j]._ParVec[k]._strParType, packetNamesList, selffoldername);
						}
					}
			...
			for (int j = 0; j < (int)m_NamespaceVec[i]._VectorVec.size(); j++) {
				if (m_NamespaceVec[i]._VectorVec[j]._name.compare(strParType.c_str()) == 0) {
					...
					if (selffoldername.compare(m_NamespaceVec[i]._protocolname.c_str()) == 0)
						...
						if (m_bTScriptUseNameSpaceVersion)
							sprintf(tt_ch, "import %s=%s.%s;", m_NamespaceVec[i]._VectorVec[j]._name.c_str(), m_NamespaceVec[i]._protocolname.c_str(), m_NamespaceVec[i]._VectorVec[j]._name.c_str());
						else
							sprintf(tt_ch, "import %s=%s.%s;", m_NamespaceVec[i]._VectorVec[j]._name.c_str(), m_NamespaceVec[i]._shortname.c_str(), m_NamespaceVec[i]._VectorVec[j]._name.c_str());
					...
					if (!IsTScriptBaseType(m_NamespaceVec[i]._VectorVec[j]._ParTName, false, true)) {
						GetUsePacketNamesByParType_TScript_NSV(m_NamespaceVec[i]._VectorVec[j]._ParTName, packetNamesList, selffoldername);
					}
					return;
				}
			}
			...
			for (int j = 0; j < (int)m_NamespaceVec[i]._WJSVectorVec.size(); j++) {
				if (m_NamespaceVec[i]._WJSVectorVec[j]._name.compare(strParType.c_str()) == 0) {
					...
					if (selffoldername.compare(m_NamespaceVec[i]._protocolname.c_str()) == 0)
						...
						if (m_bTScriptUseNameSpaceVersion)
							sprintf(tt_ch, "import %s=%s.%s;", m_NamespaceVec[i]._WJSVectorVec[j]._name.c_str(), m_NamespaceVec[i]._protocolname.c_str(), m_NamespaceVec[i]._WJSVectorVec[j]._name.c_str());
						else
							sprintf(tt_ch, "import %s=%s.%s;", m_NamespaceVec[i]._WJSVectorVec[j]._name.c_str(), m_NamespaceVec[i]._shortname.c_str(), m_NamespaceVec[i]._WJSVectorVec[j]._name.c_str());
					}


					AddPacketNames(packetNamesList, tt_ch);
					if (!IsTScriptBaseType(m_NamespaceVec[i]._WJSVectorVec[j]._ParTName, false, true)) {
						GetUsePacketNamesByParType_TScript_NSV(m_NamespaceVec[i]._WJSVectorVec[j]._ParTName, packetNamesList, selffoldername);
					}
			...
			for (int j = 0; j < (int)m_NamespaceVec[i]._EnumVec.size(); j++) {
				if (m_NamespaceVec[i]._EnumVec[j]._name.compare(strParType.c_str()) == 0) {
					...
					if (selffoldername.compare(m_NamespaceVec[i]._protocolname.c_str()) == 0)
						...
						if (m_bTScriptUseNameSpaceVersion)
							sprintf(tt_ch, "import %s=%s.%s;", m_NamespaceVec[i]._EnumVec[j]._name.c_str(), m_NamespaceVec[i]._protocolname.c_str(), m_NamespaceVec[i]._EnumVec[j]._name.c_str());
						else
							sprintf(tt_ch, "import %s=%s.%s;", m_NamespaceVec[i]._EnumVec[j]._name.c_str(), m_NamespaceVec[i]._shortname.c_str(), m_NamespaceVec[i]._EnumVec[j]._name.c_str());
					}
	...
	for (size_t i = 0; i < m_pRefProtocolParseList.size(); i++) {
		if (m_pRefProtocolParseList[i])
			((ProtocolParse_Writer_TypeScript*)m_pRefProtocolParseList[i])->GetUsePacketNamesByParType_TScript_NSV(strParType, packetNamesList, selffoldername, false);
	}

}
void    ProtocolParse_Writer_TypeScript::GetUsePacketNamesByParType_TScript_reference(const string& strParType, vector<string>& packetNamesList, const string& selffoldername)
{
	for (int i = 0; i < (int)m_NamespaceVec.size(); i++) {
		for (int j = 0; j < (int)m_NamespaceVec[i]._StructVec.size(); j++) {
			if (m_NamespaceVec[i]._StructVec[j]._name.compare(strParType.c_str()) == 0) {
				...
				if (selffoldername.compare(m_NamespaceVec[i]._protocolname.c_str()) == 0)
					...
					sprintf(tt_ch, "///<reference path=\"../%s/%s.ts\"/>", m_NamespaceVec[i]._protocolname.c_str(), m_NamespaceVec[i]._StructVec[j]._name.c_str());
				AddPacketNames(packetNamesList, tt_ch);

				for (int k = 0; k < (int)m_NamespaceVec[i]._StructVec[j]._ParVec.size(); k++) {
					if (!IsTScriptBaseType(m_NamespaceVec[i]._StructVec[j]._ParVec[k]._strParType, false, true)) {
						GetUsePacketNamesByParType_TScript_reference(m_NamespaceVec[i]._StructVec[j]._ParVec[k]._strParType, packetNamesList, selffoldername);
					}
				}
		...
		for (int j = 0; j < (int)m_NamespaceVec[i]._VectorVec.size(); j++) {
			if (m_NamespaceVec[i]._VectorVec[j]._name.compare(strParType.c_str()) == 0) {
				...
				if (selffoldername.compare(m_NamespaceVec[i]._protocolname.c_str()) == 0)
					...
					sprintf(tt_ch, "///<reference path=\"../%s/%s.ts\"/>", m_NamespaceVec[i]._protocolname.c_str(), m_NamespaceVec[i]._VectorVec[j]._name.c_str());

				AddPacketNames(packetNamesList, tt_ch);

				if (!IsTScriptBaseType(m_NamespaceVec[i]._VectorVec[j]._ParTName, false, true)) {
					GetUsePacketNamesByParType_TScript_reference(m_NamespaceVec[i]._VectorVec[j]._ParTName, packetNamesList, selffoldername);
				}
				return;
			}
		}
		...
		for (int j = 0; j < (int)m_NamespaceVec[i]._WJSVectorVec.size(); j++) {
			if (m_NamespaceVec[i]._WJSVectorVec[j]._name.compare(strParType.c_str()) == 0) {
				...
				if (selffoldername.compare(m_NamespaceVec[i]._protocolname.c_str()) == 0)
					...
					sprintf(tt_ch, "///<reference path=\"../%s/%s.ts\"/>", m_NamespaceVec[i]._protocolname.c_str(), m_NamespaceVec[i]._WJSVectorVec[j]._name.c_str());

				AddPacketNames(packetNamesList, tt_ch);
				if (!IsTScriptBaseType(m_NamespaceVec[i]._WJSVectorVec[j]._ParTName, false, true)) {
					GetUsePacketNamesByParType_TScript_reference(m_NamespaceVec[i]._WJSVectorVec[j]._ParTName, packetNamesList, selffoldername);
				}
		...
		for (int j = 0; j < (int)m_NamespaceVec[i]._EnumVec.size(); j++) {
			if (m_NamespaceVec[i]._EnumVec[j]._name.compare(strParType.c_str()) == 0) {
				...
				if (selffoldername.compare(m_NamespaceVec[i]._protocolname.c_str()) == 0)
					...
					sprintf(tt_ch, "///<reference path=\"../%s/%s.ts\"/>", m_NamespaceVec[i]._protocolname.c_str(), m_NamespaceVec[i]._EnumVec[j]._name.c_str());
	...
	for (size_t i = 0; i < m_pRefProtocolParseList.size(); i++) {
		if (m_pRefProtocolParseList[i])
			((ProtocolParse_Writer_TypeScript*)m_pRefProtocolParseList[i])->GetUsePacketNamesByParType_TScript_reference(strParType, packetNamesList, selffoldername);
	}
}
void    ProtocolParse_Writer_TypeScript::WriteClass_IReceiver_TScript_NSV(stClass& pClass, int filetype, stNamespace& pNamespace, string foldername, bool bemptyimplement, const string& strWriteNameSpace)
{
	char t_ch[1024];
	...
	for (size_t i = 0; i < pClass._FuncVec.size(); i++) {
		...
		if (filetype == 2 && ((m_bSCSpliteS && pClass._FuncVec[i]._SCSpliteType == -1) || (m_bSCSpliteC && pClass._FuncVec[i]._SCSpliteType == 1)))
			continue;
		//写函数
		WriteFunc_IReceiver_TScript_NSV(pClass._FuncVec[i], filetype, 4, pClass, pNamespace, foldername, fp, bemptyimplement);
	}
...
void    ProtocolParse_Writer_TypeScript::WriteClass_Receiver_TScript_NSV(stClass& pClass, int filetype, stNamespace& pNamespace, string foldername, const string& strWriteNameSpace)
{
	char t_ch[1024];
	...
	WriteFunc_ReceiverParser_TScript_NSV(pClass, filetype, 4, pNamespace, foldername, fp);
	...
	for (size_t i = 0; i < pClass._FuncVec.size(); i++) {
		...
		if (filetype == 2 && ((m_bSCSpliteS && pClass._FuncVec[i]._SCSpliteType == -1) || (m_bSCSpliteC && pClass._FuncVec[i]._SCSpliteType == 1)))
			continue;
		WriteFunc_Receiver_TScript_NSV(pClass._FuncVec[i], filetype, 4, pClass, pNamespace, foldername, fp);
	}
...
void    ProtocolParse_Writer_TypeScript::WriteFunc_ReceiverParser_TScript_NSV(stClass& pClass, int filetype, int tablenum, stNamespace& pNamespace, string foldername, FILE* fp)
{
	char t_ch[1024];
...
void    ProtocolParse_Writer_TypeScript::WriteClass_TScript_NSV(stClass& pClass, int filetype, stNamespace& pNamespace, string foldername, const string& strWriteNameSpace)
{
	switch (filetype) {
	case 0:
	{
		...
		if (pClass._strtype.compare("callback") == 0) {
			WriteClass_IReceiver_TScript_NSV(pClass, filetype, pNamespace, foldername, false, strWriteNameSpace);
	...
	case 2:
	{
		...
		if (pClass._strtype.compare("request") == 0) //C2S
		{
			WriteClass_IReceiver_TScript_NSV(pClass, filetype, pNamespace, foldername, true, strWriteNameSpace);
			WriteClass_Receiver_TScript_NSV(pClass, filetype, pNamespace, foldername, strWriteNameSpace);
			WriteClass_Send_TScript_NSV(pClass, filetype, pNamespace, foldername, strWriteNameSpace);
		}
...
void    ProtocolParse_Writer_TypeScript::WriteFunc_IReceiver_TScript_NSV(stFunc& pFunc, int filetype, int tablenum, stClass& pClass, stNamespace& pNamespace, string foldername, FILE* fp, bool bemptyimplement)
{
	char t_ch[1024];
...
void    ProtocolParse_Writer_TypeScript::WriteFunc_Receiver_TScript_NSV(stFunc& pFunc, int filetype, int tablenum, stClass& pClass, stNamespace& pNamespace, string foldername, FILE* fp)
{
	char t_ch[1024];
